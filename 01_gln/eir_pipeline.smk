"""
Snakemake pipeline to perform K-fold cross validation for hyperparameter tuning with EIR, EIR train and EIR predict
"""
...
wildcard_constraints:   
    diagnosis="skizo|autism|skizospek|majordd|bipol|adhd|recurdd|singledd",
    # diagnosis="adhd",
    control="nodiagkontrol2015I|nodiag|nocontrol",
    cohort="iPSYCH2012_EurUnrel|iPSYCH2015i_EurUnrel",
    input_type="onlygen|onlycov|gencov",
    threshold="0.05" 
# TODO: keep for Informed SNP set at 0.2, uncomment when running on subsets of SNPs


localrules: all, random_hyper_combs, write_configs, process_configs_for_eirtrain_pretrained, create_combinations_input_configs, process_configs_for_eirpredict
rule all:
    input:

        expand("iPSYCH2015i_{tabdata}_{diagnosis}_{control}_{cohort}_{task_type}_{input_type}_{suffix_output_folder}_hypercomb_{hypercomb}_valid_{fold}_thresh_{threshold}/eirpredict.done",
...
rule eirtrain:
    input:
        subset_snps_file = "/home/leocob/igpv/backup/iPSYCH2012_EurUnrel/genome_wide_PRS/psych_P_thresh_{threshold}_iPSYCH2012_EurUnrel_clumped_r2_0.5.rsids",
...
rule create_combinations_input_configs:
    ...
    run:
        ...
        for feature in feature_set.split('-'):
            ...
            # print("Feature_to_add: {feature_to_add}".format(feature_to_add=features_to_add))
        
            
            columns_for_diagnosis.extend(features_to_add)
            # change the following by matching it with the diagnosis



        input_config['input_info']['input_source'] = os.path.join(metadata_path, diagnosis, "genotypes", "csv", f"train_valid_{diagnosis}_nodiagkontrol2015I_{cohort}_pgss_fgrss_bigstatsr.csv")
        new_yaml_data = {
...
rule process_configs_for_eirtrain_pretrained:
    ...
    run:
        ...
        with open(input.fusion_configs) as f:
            ...
            if params.interactions == "off":
                model_type = "identity"
            elif params.interactions == "on":
                model_type = "mlp-residual"

            fusion_configs["model_type"] = model_type
        ...
        if params.interactions == "off":
            model_type = "linear"
            del output_configs["model_config"]["model_init_config"]


        elif params.interactions == "on":
            model_type = "mlp_residual"

        output_configs["model_config"]["model_type"] = model_type
        # from output_configs["model_config"] remove "model_init_config"
...
rule process_configs_for_eirpredict:
    input:
        global_configs = "{tabdata}_{diagnosis}_{control}_{cohort}_{task_type}_{input_type}_{suffix_output_folder}_hypercomb_{hypercomb}_valid_{fold}_thresh_{threshold}/configs/global_config.yaml",
        fusion_configs = "{tabdata}_{diagnosis}_{control}_{cohort}_{task_type}_{input_type}_{suffix_output_folder}_hypercomb_{hypercomb}_valid_{fold}_thresh_{threshold}/configs/fusion_config.yaml",
        output_configs = "{tabdata}_{diagnosis}_{control}_{cohort}_{task_type}_{input_type}_{suffix_output_folder}_hypercomb_{hypercomb}_valid_{fold}_thresh_{threshold}/configs/output_configs.yaml",
        input_configs = "{tabdata}_{diagnosis}_{control}_{cohort}_{task_type}_{input_type}_{suffix_output_folder}_hypercomb_{hypercomb}_valid_{fold}_thresh_{threshold}/configs/input_configs.yaml"


    output:
        global_configs = "iPSYCH2015i_{tabdata}_{diagnosis}_{control}_{cohort}_{task_type}_global_config_{input_type}_{suffix_output_folder}_hypercomb_{hypercomb}_thresh_{threshold}_valid_{fold}.yaml",
        fusion_configs = "iPSYCH2015i_{tabdata}_{diagnosis}_{control}_{cohort}_{task_type}_fusion_config_{input_type}_{suffix_output_folder}_hypercomb_{hypercomb}_thresh_{threshold}_valid_{fold}.yaml",
        output_configs = "iPSYCH2015i_{tabdata}_{diagnosis}_{control}_{cohort}_{task_type}_output_config_{input_type}_{suffix_output_folder}_hypercomb_{hypercomb}_thresh_{threshold}_valid_{fold}.yaml",
        input_configs = "iPSYCH2015i_{tabdata}_{diagnosis}_{control}_{cohort}_{task_type}_input_config_{input_type}_{suffix_output_folder}_hypercomb_{hypercomb}_thresh_{threshold}_valid_{fold}.yaml",
        input_configstabs = "iPSYCH2015i_{tabdata}_{diagnosis}_{control}_{cohort}_{task_type}_input_configtabs_{input_type}_{suffix_output_folder}_hypercomb_{hypercomb}_thresh_{threshold}_valid_{fold}.yaml"
     

    log:
        "logs/process_configs_for_eirpredict_iPSYCH2015i_{tabdata}_{diagnosis}_{control}_{cohort}_{task_type}_{input_type}_{suffix_output_folder}_hypercomb_{hypercomb}_thresh_{threshold}_valid_{fold}.log"
    ...
    run:
        ...
        # Read input.fusion_configs
        with open(input.fusion_configs) as f:
            fusion_configs = yaml.load(f, Loader=yaml.FullLoader)
        # Write output.fusion_configs
        with open(output.fusion_configs, 'w') as f:
            yaml.dump(fusion_configs, f)
        ...
        global_configs["manual_valid_ids_file"] = global_configs["manual_valid_ids_file"].replace("iPSYCH2012", "iPSYCH2015i")


        if params.attributions == "off":
            global_configs["compute_attributions"] = False
        elif params.attributions == "on":
            global_configs["compute_attributions"] = True
            global_configs["attribution_background_samples"] = params.batch_size
            # global_configs["batch_size"] = 32

        with open(output.global_configs, 'w') as f:
            yaml.dump(global_configs, f)

        # read output.output_configs
        with open(input.output_configs) as f:
            output_configs = yaml.load(f, Loader=yaml.FullLoader)

        output_configs = output_configs[0]
        train_valid_output_source = output_configs["output_info"]["output_source"]
        iPSYCH2015i_output_source = f"/faststorage/jail/project/igpv/backup/metadata/iPSYCH2015i_EurUnrel/{wildcards.diagnosis}/all_splits_genotypes_{wildcards.diagnosis}_nodiagkontrol2015I_iPSYCH2015i_EurUnrel.csv"
        iPSYCH2015i_input_source_tabs = f"/faststorage/jail/project/igpv/backup/metadata/iPSYCH2015i_EurUnrel/{wildcards.diagnosis}/all_splits_genotypes_{wildcards.diagnosis}_nodiagkontrol2015I_iPSYCH2015i_EurUnrel_pgss_fgrss_bigstatsr.csv"

        # print(test_output_source)
        # test_output_name = train_valid_output_name.replace("valid", "test")
        # output_configs["output_info"]["output_source"] = iPSYCH2015i_output_source
        ...
        input_configs = input_configs[0]


        iPSYCH2012_input_source = input_configs["input_info"]["input_source"]
        iPSYCH2012_snp_file = input_configs["input_type_info"]["snp_file"]
        iPSYCH2015i_input_source = iPSYCH2012_input_source.replace("iPSYCH2012", "iPSYCH2015i")
        iPSYCH2015i_snp_file = iPSYCH2012_snp_file.replace("iPSYCH2012", "iPSYCH2015i")
        input_configs["input_info"]["input_source"] = iPSYCH2015i_input_source
        input_configs["input_type_info"]["snp_file"] = iPSYCH2015i_snp_file
       
        with open(output.input_configs, 'w') as f:
            yaml.dump(input_configs, f)
        
        input_configstab["input_info"]["input_source"] = iPSYCH2015i_input_source_tabs
...
rule eirpredict_test:
    ...
    output:
        output_dir = directory("test_{tabdata}_{diagnosis}_{control}_{cohort}_{task_type}_{input_type}_{suffix_output_folder_continued}_hypercomb_{hypercomb}_valid_{fold}_thresh_{threshold}"),
        output_touch = touch("test_{tabdata}_{diagnosis}_{control}_{cohort}_{task_type}_{input_type}_{suffix_output_folder_continued}_hypercomb_{hypercomb}_valid_{fold}_thresh_{threshold}/eirpredict.done")
    ...
    params:
        model_file = "{tabdata}_{diagnosis}_{control}_{cohort}_{task_type}_{input_type}_{suffix_output_folder_continued}_hypercomb_{hypercomb}_valid_{fold}_thresh_{threshold}/saved_models/*.pt"



    log:
        "logs/eirpredict_{tabdata}_{diagnosis}_{control}_{cohort}_{task_type}_{input_type}_{suffix_output_folder_continued}_hypercomb_{hypercomb}_thresh_{threshold}_valid_{fold}.log"
...
rule eirpredict_iPSYCH2015i:
    input:
        ...
        input_configs = rules.process_configs_for_eirpredict.output.input_configs,
        input_configtabs = rules.process_configs_for_eirpredict.output.input_configstabs
        configs = "test_{tabdata}_{diagnosis}_{control}_{cohort}_{task_type}_input_config_{input_type}_{suffix_output_folder}_hypercomb_{hypercomb}_thresh_{threshold}_valid_{fold}.yaml"

    output:
        output_dir = directory("iPSYCH2015i_{tabdata}_{diagnosis}_{control}_{cohort}_{task_type}_{input_type}_{suffix_output_folder}_hypercomb_{hypercomb}_valid_{fold}_thresh_{threshold}"),
        output_touch = touch("iPSYCH2015i_{tabdata}_{diagnosis}_{control}_{cohort}_{task_type}_{input_type}_{suffix_output_folder}_hypercomb_{hypercomb}_valid_{fold}_thresh_{threshold}/eirpredict.done")
    ...
    log:
        "logs/eirpredict_iPSYCH2015i_{tabdata}_{diagnosis}_{control}_{cohort}_{task_type}_{input_type}_{suffix_output_folder}_hypercomb_{hypercomb}_thresh_{threshold}_valid_{fold}.log"
